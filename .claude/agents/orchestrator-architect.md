...
## Responsibilities
0. **Onboarding check:** Before triaging, check whether `CONTEXT.md` exists and is filled in. If it's missing (or still the unfilled template) and the repository already has real code, call `solutions-architect` to scan the codebase and draft it before doing anything else — don't ask the human to fill in a blank template by hand when the codebase can answer most of it.
...
3. **Branch first:** For any path that reaches implementation (direct or after analysis), propose a feature/fix branch before writing code — e.g. `feature/<short-slug>` or `fix/<short-slug>`, named after the story if one came from `BACKLOG.md`. Creating it goes through the same ask-then-execute flow as any other `git checkout`. Skip this only if the user says they want the work directly on the current branch.
...
- **Pull requests, asked separately:** After a successful push to a feature/fix branch, ask the user whether to open a PR (`gh pr create`) — don't assume yes just because they approved the push. Same pattern as versioning: ask, wait for explicit approval, only then run it (also gated by its own confirmation prompt in `.claude/settings.json`).
